...
`localStorage`. Persistence is the caller's job: `serializeChipDefinition`
produces plain JSON, `deserializeChipDefinition` validates and rebuilds —
...
A `ChipDefinition` (custom AND, HalfAdder, ALU, …) is a plain record: id,
name, `PortDefinition[]` for its interface, and a `CircuitDefinition` for
its internals. There is no `AndChip` class and no growing `Component` →
`Chip` → `BinaryChip` inheritance chain. `NAND` is the sole exception: it's
a built-in `PrimitiveChipBehavior` with a direct truth-table function,
because it has no internal circuit to decompose into.

`ChipRegistry.resolve(type)` is the single place that tells a primitive
from a custom chip apart. Everywhere else — `Circuit`, `validateConnection`,
`evaluateCircuit` — only sees the resulting `ResolvedChipType` union and
branches on `.kind`, never on a hardcoded list of chip names. Adding a new
custom chip never requires touching the evaluator.

## 3. The ChipRegistry

A `ComponentDefinition` only ever stores `{ id, type }` — `type` is a bare
string, never an embedded copy of a chip's behavior or internal circuit.
That keeps `CircuitDefinition` flat and serializable: a chip used in ten
places is referenced by its id ten times, not duplicated ten times, and a
`ComponentDefinition` never holds a live reference into another chip's
data. `ChipRegistry` is what turns that string back into something
...
(`chip/ChipRegistry.ts`):

- `registerPrimitive(behavior)` — registers a built-in `PrimitiveChipBehavior`
  (truth-table function) under its own `type` name, e.g. `"NAND"`.
- `registerChip(definition)` — registers a custom `ChipDefinition` under its
  `id`, e.g. the id `createChipDefinition` generated for a custom `AND`.
- `resolve(type)` — looks in primitives first, then custom chips, returns
  a `ResolvedChipType` (`{ kind: "primitive", ... }` or
  ...
  that tells a primitive from a custom chip apart.
...
- `hasChip(id)` — lets a caller avoid re-registering a chip it may have
  already registered.
...
`ChipDefinition`, auto-registering the latter (`registerChip`, guarded by
`hasChip`) so it can immediately be wired up and later resolved during
`evaluateCircuit`. Composing chips (a custom chip built from other custom
chips, as in the adder e2e tests) means passing the **same** registry
instance all the way down, so a chip registered while building one circuit
is still visible when `evaluateCircuit` later resolves it as a component
somewhere else.

Because the registry is runtime-only, restoring a saved chip is not just
`deserializeChipDefinition` on its own bytes: any custom chip it uses as a
component must be separately serialized and re-registered into the
registry `evaluateChip` will use, or `resolve` throws
...
**The problem.** A `ChipDefinition` for, say, a custom `AND` chip has two
...
- `BOUNDARY_ID.A` is the _source_ of an arrow (it feeds `nand1.A`), so for
  ...
  an **input** port on the `AND` chip's own `PortDefinition`.
...
So: **from outside the chip**, `A`/`B` are inputs you feed values into and
`Y` is an output you read a value from — completely ordinary. **From
inside the internal wiring**, that's flipped: the chip's own inputs behave
like sources (things that hand a value in, i.e. outputs), and the chip's
...
declared direction (`direction: isChipInput ? "output" : "input"`) purely
so `ruleDirection` sees a valid output→input arrow. Nothing about the
`PortDefinition` itself, or how the chip looks from outside, changes.
...
- **Definition** (`ChipDefinition`, `CircuitDefinition`, `ComponentDefinition`,
  ...
  object reference. This is the only thing a saved custom chip contains.
...
`serializeChipDefinition`/`deserializeChipDefinition` (in
`src/core/serialization/`) define a deliberate versioned format
(`SerializedChipDefinitionV1`, `version: 1`), not `JSON.stringify(chip)`.
...
new `SerializedChipDefinitionV*` and branch in `deserializeChipDefinition`
...
sequential logic can be layered on without reshaping `ChipDefinition` or
`CircuitDefinition`.
